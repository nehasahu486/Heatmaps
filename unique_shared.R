setwd('C:/Users/Dell/Documents/TopGO/Armi_compare/unique_shared/')
list.files()
rm(list=ls())
library(tidyr)
library(dplyr)
...
og<-read.delim("ortho3col_67sps.tsv")
head(og)
unique(og$Species)
...
og1<-left_join(og,lf,by="Species")
head(og1)
og2<-data.frame(table(og1$OG_ID,og1$LF))
head(og2)
og2$Count<-ifelse(og2$Freq>0,"1","0") #we need 0/1 matrix for upsetR
head(og2)
tail(og2)

og3<-dcast(og2[c(1,2,4)],Var1~Var2,value="Count")
head(og3)
write.table(og3,"OG_counts_matrix.tsv",quote=F,row.names=F,sep="\t")

####################################################################
####################################################################
#############_____________START FROM HERE________###################
####################################################################
####################################################################
order<-read.delim("Order_of_sps.tsv")
head(order)  
ord<-(order$Species)
ord

up<-read.delim("Armi_for_upset.tsv")
head(up)
